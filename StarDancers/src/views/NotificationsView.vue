...
import TableList from "@/components/NotificationsTable/TableList.vue";
import Modal from "@/components/NotificationsTable/ModalTable.vue";
...
export default {
  ...
  methods: {
    async get() {
      return ([this.notifications, this.pagination] = await getNotifications(
        this.page,
        this.query
      ));
    },
  ...
  async beforeMount() {
    [this.notifications, this.pagination] = await this.get();
  },
...
<template class="bg-slate-100">
  ...
  >
    ...
    >
      <div class="w-full md:grid md:grid-cols-3 gap-3 px-16">
        <div class="md:col-span-2">
          <header>
            ...
            <p class="text-md pt-2 font-medium subtitle max-sm:text-center">
              Um total de {{ pagination.total }} notificações. 🔔
            </p>
          </header>
        </div>
        <div
        ...
        >
          <div class="max-sm:w-full">
            <button
              type="button"
              class="flex items-center justify-center gap-2 text-white transition w-full rounded bg-pink-600 px-6 py-2.5 cursor-pointer font-medium text-sm mr-2 mb-2"
      ...
      <TableList
        ...
        :loading="loading"
        :show="(item: any) => ((notification = item), (showModal = true))"
  ...
  <modal
    v-if="showModal"
    :close="() => ((showModal = false), (notification = {}), get())"
