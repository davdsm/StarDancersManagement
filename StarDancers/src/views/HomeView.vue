...
<template class="bg-slate-100">
  <div class="flex-1 h-auto transition-all duration-300 rounded-lg mx-[20px] w-[calc(100%-40px)] md:w-4/6">
    <div class="entry flex flex-col bg-white pt-12 pb-12 shadow-davdsm rounded-lg delay" :class="!isAdmin && 'mb-20'">
    ...
    <PaginationTable v-if="isAdmin" :page="page" :pagination="pagination" :handlePagination="handlePagination" />
  </div>
  <modal v-if="showModal" :close="() => (showModal = false)" :update="update" :delete="delete" :create="create"
    :pay="pay" v-bind:student="student" />
</template>
