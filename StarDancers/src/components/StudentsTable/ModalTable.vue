...
import { createUser, updateUserPassword } from "@/services/auth";
...
export default {
  ...
  data() {
    return {
      ...
      successMsg: <boolean | string>false,
  ...
  methods: {
    ...
    async handlePasswordRecovery(password: string, confirmPassword: string) {
      if (password !== confirmPassword) {
        this.errorMsg = "As passwords não coincidem";
        return;
      }

      if (password.length < 6) {
        this.errorMsg = "A password deve ter pelo menos 6 caracteres";
        return;
      }

      try {
        const email = this.local_student.attributes.ParentEmail;
        ...
        this.successMsg = "Password atualizada com sucesso!";
      } catch (error) {
        this.errorMsg = "Erro ao atualizar a password";
      }
    },
  },
  components: { DeletePoup, ErrorModal, SuccessModal, PasswordRecoveryModal },
  async mounted() {
...
<template>
  ...
  >
    ...
    ></div>
    <div
      class="md:h-auto max-h-[calc(100%-2rem)] mt-5 relative w-full max-w-2xl h-full overflow-y-auto overflow-x-hidden z-50 bg-white rounded-lg dark:bg-slate-800"
    >
      <!-- Modal content -->
      <form
        action="#"
      ...
      >
        <!-- Modal header -->
        <div
          class="flex justify-between items-center p-4 rounded-t border-b gap-4 dark:border-slate-700"
        >
          <h3
          ...
          <div class="flex gap-2">
            ...
            <span
              v-if="isAdmin"
              @click="isAdmin ? (passwordRecoveryModal = true) : null"
              class="ml-auto px-4 py-2 font-bold bg-slate-200 rounded text-xs text-slate-800 flex items-center gap-2"
              :class="isAdmin ? 'cursor-pointer' : 'cursor-initial'"
            >
              Recuperar Password
              <svg
        ...
        <div class="p-6 space-y-6">
          <div class="grid grid-cols-6 gap-6">
            ...
            <div class="col-span-6 sm:col-span-3">
              ...
              <input
                ...
                :disabled="!isAdmin || local_student.id !== 0"
                required
              />
            </div>
            <div class="col-span-6 sm:col-span-3">
            ...
            <div class="col-span-6 sm:col-span-3">
              ...
              <textarea
                id="message"
                rows="4"
                class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-slate-700 dark:border-slate-600 dark:placeholder-gray-400 dark:text-white"
          ...
          <div class="grid grid-col-2 grid-flow-col gap-3">
            <div>
              <div
                class="flex items-center pl-4 rounded border border-gray-200 dark:border-[#161e32]"
              >
                <input
                  ...
                  class="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 focus:ring-blue-500 dark:ring-offset-gray-800 dark:bg-gray-700 dark:border-gray-600"
                  :disabled="!isAdmin"
                />
                <label
                  for="bordered-checkbox-1"
                  class="py-4 ml-2 w-full block text-sm font-medium text-gray-900 dark:text-gray-300"
          ...
          <div v-if="local_student.id === 0">
            ...
            >
              ...
              <label
                ...
                >Aceito os
                <a
                  class="text-sm font-medium text-gray-900 underlined underline decoration-indigo-500 dark:text-gray-300"
                  href=""
                  target="_blank"
    ...
    <div
      ...
      class="mt-5 relative w-1/5 bg-white rounded-lg z-50 flex justify-center items-center flex-col gap-4 dark:bg-slate-800"
    >
      <div
        class="flex justify-between items-center p-4 rounded-t border-b gap-4 dark:border-slate-700"
      >
  ...
  <DeletePoup
    ...
    :yes="() => that.delete(student.id, student.attributes.ParentEmail)"
  />
  <ErrorModal
    v-if="errorMsg"
    :msg="errorMsg"
  ...
  <SuccessModal
    v-if="successMsg"
    :msg="successMsg"
    :close="() => (successMsg = false)"
  />
  <PasswordRecoveryModal
    v-if="passwordRecoveryModal"
    :close="() => (passwordRecoveryModal = false)"
